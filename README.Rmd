...
Once you've installed the package you'll also need to download some icons! Supported icon libraries (listed above) can be downloaded using the `download_*()` functions. For example, to download the Font Awesome icons you would use `download_fa()`.

# Usage

Icons can be inserted inline using inline code `` `r knitr::inline_expr('icon::fa("rocket")')` `` `r icon::fa("rocket")` or `` `r knitr::inline_expr('icon::fa$rocket')` `` `r icon::fa$rocket`. 
